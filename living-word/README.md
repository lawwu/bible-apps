...
the King James Version with its most cross-referenced verses highlighted.
...
- **Click a node** to focus it: a chapter shows its most-referenced verses
  (with the KJV text), strongest connected chapters, and the doctrines,
...
- **Read this chapter** (or click any verse row) opens the reader: the full
  KJV chapter with its most cross-referenced verses highlighted and marked
...
| `data/topics.json` | Top 150 OpenBible Topical Bible topics (CC-BY openbible.info) with top-voted verses and KJV text |
| `data/text/{1..66}.json` | KJV text per book (via scrollmapper/bible_databases, shared `verse-explorer` copy), fetched on demand by the reader |

### Pipeline

```sh
python3 scripts/build_graph.py    # TSK -> graph.json + details.json
python3 scripts/build_text.py    # verse-explorer KJV -> data/text/{book}.json
