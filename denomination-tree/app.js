/* The Denomination Tree — renders the genealogy in data.js. No dependencies.
   ...
   Clicking a church lights its whole lineage — ancestors and heirs. */

const COLS = [110, 250, 390, 530, 660, 800, 950, 1110];
const W = 1240;
const MARGIN_TOP = 26;
const NODE_H = 42;
const MIN_GAP = 56;
...
const Y_DOMAIN = [33, 451, 1054, 1517, 1650, 1790, 1900, 2026];
const Y_RANGE  = [0,  120, 260,  420,  760,  980,  1200, 1900];
function yScale(v) {
...
svg.setAttribute("aria-label", "Family tree of Christian denominations, AD 33 to today");
...
ERAS.forEach((d, i) => {
  ...
  el("text", { class: "era-name", x: W - 14, y: yScale(d.from) + 19 }, g,
    `${i === 0 ? "AD 33" : d.from} · ${d.name.toUpperCase()}`);
...
for (const d of NODES) {
  ...
  if (d.adh) {
    const bw = Math.max(6, Math.sqrt(d.adh) * 4.2);
    el("rect", {
      class: "size-bar", x: -bw / 2, y: NODE_H / 2 + 7,
      width: bw, height: 3, rx: 1.5,
      fill: STREAMS[d.stream].color, opacity: 0.55,
    }, g);
  }
  d.el = g;
...
function fmtAdh(m) {
  if (m >= 1000) return `~${(m / 1000).toFixed(1)} billion`;
  if (m >= 1) return `~${Math.round(m)} million`;
  return `~${Math.round(m * 1000).toLocaleString()},000`;
}
...
function select(id, scroll = true) {
  ...
  if (id) {
    ...
    document.getElementById("detail").innerHTML =
      `<p class="detail-hint">Click any church above to trace its whole lineage and read its story here.</p>`;
  }
}
...
function showTip(event, d) {
  ...
  tooltip.innerHTML = `<span class="tip-ref">${d.years} · ${PHIL[d.phil]}${d.adh ? " · " + fmtAdh(d.adh) : ""}</span><b>${d.name}</b> — ${teaser}`;
...
function baseLine(d) {
  return d.parents.map(([pid, kind]) => {
    ...
    const verb = p.phil === "source" ? "descends from"
      : kind === "merge" ? "formed by union within"
      : kind === "rev" ? "separated from" : "shaped by";
    return `${verb} ${p.name}`;
  }).join(" · ");
}
...
function showDetail(d) {
  ...
  document.getElementById("detail").innerHTML = `
    ...
    <p class="detail-dates">${d.years}${d.adh ? ` · ${fmtAdh(d.adh)} adherents today` : ""}${d.parents.length ? " · " + baseLine(d) : ""}</p>
...
tbody.innerHTML = [...NODES]
  ...
  .map((n) => `<tr>
    ...
    <td>${PHIL[n.phil]}</td>
    <td>${n.adh ? fmtAdh(n.adh) : "—"}</td>
    <td>${n.parents.map(([pid, kind]) => {
      const p = byId.get(pid);
      return `${p.phil === "source" ? "from" : kind === "merge" ? "union within"
        : kind === "rev" ? "separated from" : "shaped by"} ${p.short}`;
    }).join("; ")}</td>
  </tr>`).join("");
